...
class Address(models.Model):
    ...
    def __str__(self):
        return f"{self.id} - {self.address_line1}, {self.city}, {self.country}"
...
class UserAddress(models.Model):
    ...
    def __str__(self):
        return f"{self.user.email} - {self.address.address_line1}, {self.address.city}, {self.address.country}"
    
class PickupStation(models.Model):
    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='pickup_stations')
    name = models.CharField(max_length=100)
    address = models.ForeignKey(Address, on_delete=models.CASCADE, related_name='pickup_stations')
    contact_number = models.CharField(max_length=15, blank=True, null=True)
    email = models.EmailField(blank=True, null=True)
    operating_hours = models.CharField(max_length=255, blank=True, null=True)

    def __str__(self):
        return f"{self.name} - {self.address.address_line1}, {self.address.city}, {self.address.country}"